...
This section is designed for experienced users who need to manage complex configurations and orchestration settings in Nomploy. The **Advanced** tab is located in your **Applications** settings. Here, you can execute custom commands, manage cluster replicas, select Docker registries, and configure Docker Swarm settings.
...
1. **Build Phase**: Nomploy connects to your build server via SSH, clones your repository, and builds the Docker image on the build server
...
<Callout type="warn">
  **Required Configuration**: When using a build server, you must configure a Docker registry in your Nomploy settings. The built image needs to be stored in a registry that's accessible to your deployment servers. See [Docker Registry](/docs/core/registry) for configuration details.
</Callout>
