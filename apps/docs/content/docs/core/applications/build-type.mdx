---
title: Build Type
description: "Learn about the different build types available in Nomploy, including Nixpacks, Dockerfile, and Buildpack options."
...
Nomploy offers three distinct build types for deploying applications, each suited to different development needs and preferences.

### Nixpacks

This is the default build type in Nomploy. When you select Nixpacks, Nomploy builds your application as a Nixpack, which is optimized for ease of use and efficiency.
...
Since Nixpacks have a [static builder](https://nixpacks.com/docs/providers/staticfile) Nomploy expose a field called `Publish Directory`  where basically you can specify
the directory that you want to publish after the build process is finished, example:

Astro applications after you build it usually create a `dist` directory, so you can specify the `dist` directory as the publish directory and then Nomploy will
...
Nomploy provides a **Railpack Version** field in the application settings where you can specify a specific version of Railpack to use. This allows you to:
...
4. Nomploy will automatically download and use the specified version for your builds
...
<Callout type="warn">
  If you specify an invalid version, Nomploy will show an error. Make sure to use a valid version from the [Railpack releases page](https://github.com/railwayapp/railpack/releases).
</Callout>
...
If your project includes a Dockerfile, you can specify its path. Nomploy will use this Dockerfile to build your application directly, giving you full control over the build environment and dependencies

Nomploy expose 3 Fields to be configured:
...
Nomploy supports two types of buildpacks:

- **Heroku**: Adapted from Heroku's popular cloud platform, these buildpacks are designed for compatibility and ease of migration, you can optional specify the Heroku Version to use, by default Nomploy will use the 24.
...
Nomploy will copy everything from the `Root` directory and will mount it to the `/usr/share/nginx/html` directory, and will use a NGINX Optimized Dockerfile to run your application.
