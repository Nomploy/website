...
description: Learn how to deploy your application in production in Nomploy.
---

import { Callout } from 'fumadocs-ui/components/callout';

By default, nomploy offer multiple [Builds Types](/docs/core/applications/build-type) to deploy your application, the most common is `nixpacks` and `heroku buildpacks`
...
1. **Use Git Provider in Your Application**:
   - Repository: `https://github.com/Nomploy/production-example`
   - Branch: `main`
   - Build path: `/`
...
9. Now let's create application in Nomploy.
...
jobs:
  build-and-push-dockerfile-image:
    ...
    steps:
      ...
      - name: Trigger Nomploy Deployment
        uses: nomploy/nomploy-action@v1
        run: |
            curl -X 'POST' \
            'https://<your-nomploy-domain>/api/application.deploy' \
...
You can also use this Github Action [Action](https://github.com/marketplace/actions/nomploy-deployment) to automate the deployment.

## Healthcheck & Rollbacks

When using Nomploy you can also configure healthchecks and rollbacks, this will allow you to configure your application to be able to recover from failures.
...
Go to the `Advanced` tab and open Cluster Settings.
