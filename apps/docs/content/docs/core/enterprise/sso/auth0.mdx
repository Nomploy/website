...
1. In Nomploy, go to **Settings** (or **Organization** / **Security** in Enterprise).
...
3. Enter:
   ...
   - **Domain**: the domain users use to authenticate via Auth0 (e.g. your organization domain like `acme.com`), not the Nomploy instance URL
...
2. Set **Allowed Callback URLs** to your Nomploy URL, for example:
   - `https://your-nomploy-domain.com/api/auth/callback/myorg-name-auth0`
3. Set **Allowed Logout URLs** to:
   - `https://your-nomploy-domain.com`
...
- **Redirect URI mismatch** — Ensure the callback URL in Nomploy matches exactly what is configured in Auth0 (including protocol and path).
...
2. Go to **Applications** → **Applications** → **Create Application**.
3. Choose **Regular Web Application** and create it.
4. In the application, go to **Add Ons** → enable **SAML 2 Web App** and configure it, in the settings specify this callback URL: `https://your-nomploy-domain.com/api/auth/sso/saml2/callback/myorg-name-auth0-saml`.
5. Next & Save.
...
1. In your Auth0 SAML application, set the **Application Callback URL** (ACS URL) to your Nomploy SAML ACS URL, for example:
   - `https://your-nomploy-domain.com/api/auth/sso/saml2/callback/myorg-name-auth0-saml`
2. In the **SAML 2 Web App** add-on, open **Settings** and paste the following JSON in the **Settings** (Application Settings) field. Replace `https://your-nomploy-domain.com` with your Nomploy base URL and `myorg-name-auth0-saml` with the **exact same provider name** you entered in Nomploy in step 2 (the callback URL path must match), so Nomploy can read email, display name, and other attributes:

```json
{
  "audience": "https://your-nomploy-domain.com/saml/metadata",
  "recipient": "https://your-nomploy-domain.com/api/auth/sso/saml2/callback/myorg-name-auth0-saml",
  "destination": "https://your-nomploy-domain.com/api/auth/sso/saml2/callback/myorg-name-auth0-saml",
  "signResponse": true,
  "signAssertion": true,
...
- **ACS URL mismatch** — Ensure the callback/ACS URL in Auth0 matches exactly what Nomploy provides (including protocol and path).
- **Certificate** — Use the full x509 certificate from Auth0 (PEM format); ensure no extra spaces or line breaks.
- **Entity ID** — The Entity ID in Nomploy must match the Issuer/Entity ID configured in Auth0.

</Tab>
</Tabs>

For help with your setup, [contact us](https://nomploy.com/contact).
