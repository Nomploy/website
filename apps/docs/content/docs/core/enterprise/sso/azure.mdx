...
3. Enter a **Name** (e.g. Nomploy), choose supported account types, and set **Redirect URI** to **Web** with a placeholder for now (e.g. `https://your-nomploy-domain.com/api/auth/callback/myorg-name-azure`).
...
3. Enter:
   ...
   - **Domain**: the domain users use to authenticate via Azure AD (e.g. your organization domain like `acme.com`), not the Nomploy instance URL
...
2. Under **Web** → **Redirect URIs**, add:
   - `https://your-nomploy-domain.com/api/auth/callback/myorg-name-azure`
...
- **Redirect URI mismatch** — Ensure the callback URL in Nomploy matches exactly what is configured in Azure (including protocol and path). Use the same **Provider** value in the path (e.g. `.../api/auth/callback/myorg-name-azure`).
...
3. Enter a **Name** (e.g. Nomploy) and create.
...
2. Under **Basic SAML Configuration**, set **Identifier (Entity ID)** if required (SP Entity ID from Nomploy) (eg. `https://your-nomploy-instance.com`).
3. Set **Reply URL (Assertion Consumer Service URL)** to your Nomploy SAML ACS URL (eg. `https://your-nomploy-instance.com/api/auth/sso/saml2/callback/myorg-name-azure-saml`).
3. Save.

## Troubleshooting (SAML)

- **ACS URL mismatch** — Ensure the Reply URL (ACS) in Azure matches exactly what Nomploy provides (including protocol and path).
- **Certificate** — Use the Certificate (Base64) from Azure; paste as-is or convert to PEM if Nomploy expects PEM.
- **Entity ID** — The Entity ID in Nomploy must match the Identifier (Entity ID) of the Azure Enterprise application.
