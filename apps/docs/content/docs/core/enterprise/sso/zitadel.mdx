...
3. Give it a name (e.g. `nomploy`) and click **Next**.
...
6. Add the following **Redirect URI**:
   - `https://your-nomploy-domain.com/api/auth/sso/callback/{providerId}`
   Replace `{providerId}` with the unique identifier you will use in Nomploy (e.g. `zitadel`).
...
Nomploy will automatically fetch the OpenID Connect discovery document from `{issuer}/.well-known/openid-configuration`.

## 3. Configure Nomploy

1. In Nomploy, go to **Settings** → **SSO** → **Register OIDC Provider**.
2. Enter:
...
3. Note the **Callback URL** shown by Nomploy (e.g. `https://your-nomploy-domain.com/api/auth/sso/callback/zitadel`).
4. Save.

## 4. Verify the Redirect URI in Zitadel

Make sure the Redirect URI configured in your Zitadel application matches exactly the Callback URL shown in Nomploy:

```
https://your-nomploy-domain.com/api/auth/sso/callback/zitadel
```
...
- **Redirect URI mismatch** — The Redirect URI in Zitadel must match the Callback URL in Nomploy exactly, including the `{providerId}` path segment.
