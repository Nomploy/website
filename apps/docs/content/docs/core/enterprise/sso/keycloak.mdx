...
5. Set **Root URL** to your Nomploy base URL, e.g. `https://your-nomploy-domain.com`.
6. Save.
...
3. Enter:
   ...
   - **Domain**: the domain users use to authenticate via Keycloak (e.g. your organization domain like `acme.com`), not the Nomploy instance URL
...
2. Set **Valid redirect URIs** to your Nomploy callback URL, for example:
   - `https://your-nomploy-domain.com/api/auth/callback/my-client-id`
...
- **Redirect URI mismatch** — Ensure the callback URL in Nomploy matches exactly what is configured in Keycloak (including protocol and path). Use the same **Provider** value in the path (e.g. `.../api/auth/callback/myorg-name-keycloak`).
...
- **Attribute mapping** — If user email or name is missing, map Keycloak attributes (e.g. email, preferred_username) in Nomploy if your setup supports it.
