...
description: "Learn how to backup your volumes using Nomploy's Volume Backups feature"
---

Volume backups are essential when your service doesn't fit the traditional database backup solutions. This is common when your application uses SQLite databases or doesn't have a database at all, making Nomploy's dedicated database backup features (PostgreSQL, MySQL, MongoDB, etc.) unsuitable for your use case.
...
Since N8N uses SQLite (stored in the `n8n_data` volume), we can't use Nomploy's database backup features. Instead, we'll backup the entire volume.
...
2. **Verify the app name** in Nomploy (usually under the name of your service)
