...
## Method 1: Using Nomploy Domains (Recommended)

The easiest way to configure domains is using Nomploy's native domain management feature. This method allows you to configure domains directly through the Nomploy UI without manually editing your Docker Compose file.

<Callout type="info">
  **Recommended**: Since v0.7.0, Nomploy supports domains natively. You can configure your domains directly in the Nomploy UI through the **Domains** tab of your Docker Compose application. This is the simplest and most user-friendly approach.
</Callout>

To use this method:
1. Navigate to your Docker Compose application in Nomploy
2. Go to the **Domains** tab
3. Click **Add Domain** and configure your domain
4. Nomploy will automatically handle the routing configuration

### How It Works

At runtime, during the deployment phase, Nomploy automatically adds Traefik labels internally to your Docker Compose file. You don't need to manually add these labels - Nomploy handles this automatically based on the domains you configure in the UI.
...
- If you're not using [Isolated Deployments](/docs/core/docker-compose/utilities#isolated-deployments), Nomploy will add the `nomploy-network` to the service you selected, however you need to add `nomploy-network` to the other services to maintain connectivity.
- The Preview Compose button is useful for verifying how Nomploy will modify your compose file before deployment.
- All label generation and network configuration is handled automatically by Nomploy based on your domain settings.

For detailed instructions on using Nomploy's domain management, see the [Domains guide](/docs/core/domains).
...
<Callout type="warn">
  Manual configuration is more complex and requires editing your Docker Compose file. We recommend using Method 1 (Nomploy Domains) unless you have specific requirements that need manual Traefik label configuration.
</Callout>
...
1. Add the service to the `nomploy-network`.
...
First, we'll add the nomploy-network to our services:

<Callout type="info">
  **Tip**: If you prefer to isolate all services and avoid adding them to the `nomploy-network`, you can use the **Isolated Deployments** feature. This feature isolates all services and eliminates the need to manually add them to the nomploy-network. See the [Isolated Deployments guide](/docs/core/docker-compose/utilities#isolated-deployments) for more information.
</Callout>
...
services:
  frontend:
    ...
    labels:
      - traefik.enable=true
      - traefik.http.routers.frontend-app.rule=Host(`frontend.nomploy.com`)
  ...
  backend:
    ...
    labels:
      - traefik.enable=true
      - traefik.http.routers.backend-app.rule=Host(`backend.nomploy.com`)
...
2. `traefik.http.routers.<UNIQUE-RULE>.rule=Host('your-domain.nomploy.com')` Specifies the domain for the service
...
4. **Isolated Deployments**: As an alternative to manually adding services to `nomploy-network`, you can use the [Isolated Deployments](/docs/core/docker-compose/utilities#isolated-deployments) feature, which automatically isolates all services and handles networking configuration for you.
...
<Callout type="info">
  Remember: For most use cases, we recommend using **Method 1 (Nomploy Domains)** as it's simpler and doesn't require manual Docker Compose file editing. See the [Domains guide](/docs/core/domains) for more information.
</Callout>

If you have any further questions or need assistance, ask on our [GitHub Discussions](https://github.com/nomploy/nomploy/discussions) and we'll be happy to help.
