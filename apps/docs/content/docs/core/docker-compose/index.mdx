...
Nomploy integrates with Docker Compose to provide flexible deployment solutions. Whether you are developing locally or deploying at scale, Nomploy manages your applications through a standard `docker-compose.yml`.

### Configuration

Write a standard `docker-compose.yml` and Nomploy runs it on your infrastructure. Configure the source of your code, the build, environment, domains, volumes and deployments through the UI.
...
The code editor in Nomploy allows you to define environment variables for your Docker Compose deployment. By default, Nomploy saves these variables to a `.env` file in the same directory as your `docker-compose.yml`.
...
Monitor each service individually within Nomploy. If your application consists of multiple services, each can be monitored separately to ensure optimal performance.

### Logs

Access detailed logs for each service through the Nomploy log viewer, which can help in troubleshooting and ensuring the stability of your services.
...
- **Command**: Nomploy has a defined command to run the Docker Compose file, ensuring complete control through the UI. However, you can override it with a custom command. Note that the custom command fully replaces the default one (it does not append to it), so if you only need extra flags like `--force-recreate`, you must write the full command based on the default shown in the UI. The command always starts with `docker`, so a custom command like `compose -p <app-name> -f <compose-path> up -d --build --remove-orphans --force-recreate` will run as `docker compose ...`.
...
Nomploy supports two methods for data persistence in Docker Compose:
...
- You don't need automated backups via Nomploy's Volume Backups feature

### Method 2: Docker Named Volumes

Use Docker named volumes when you need automated backups, better portability, or Docker-managed storage. Named volumes are managed by Docker and can be backed up automatically using Nomploy's [Volume Backups](/docs/core/volume-backups) feature.

```yaml
services:
  app:
    image: nomploy/nomploy:latest
    volumes:
