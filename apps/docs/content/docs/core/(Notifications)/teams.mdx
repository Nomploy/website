...
Microsoft Teams notifications are a great way to stay up to date with important events in your Nomploy panel. You can choose to receive notifications for specific events or all events.
...
3. **Option A – Incoming Webhook (Connectors):** Search for **Incoming Webhook**, configure it with a name (e.g. `Nomploy`), and copy the generated **Webhook URL**.
...
5. Go to Nomploy **Notifications** and select **Microsoft Teams** as the notification provider.
