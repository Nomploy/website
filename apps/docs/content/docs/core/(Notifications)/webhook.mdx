...
Webhook notifications are a generic way to receive notifications from Nomploy to any HTTP endpoint. You can choose to receive notifications for specific events or all events. Notifications are sent in JSON format.
...
For testing purposes, you can use [Webhook.site](https://webhook.site) to generate a unique URL and inspect the JSON payload that Nomploy sends:

1. Go to [https://webhook.site](https://webhook.site)
2. Copy your unique webhook URL
3. Go to Nomploy **Notifications** and select **Webhook** as the notification provider
...
Nomploy sends notifications in JSON format. The payload structure includes information about the event type, timestamp, and relevant details about the action that triggered the notification.
...
- Is accessible from the internet (or from your Nomploy server's network)
