...
Telegram notifications are a great way to stay up to date with important events in your Nomploy panel. You can choose to receive notifications for specific events or all events.
...
3. Set a name for your bot, eg. `nomploy_bot` make sure the name ends with `_bot`.
4. Copy the `Bot Token` and paste it in Nomploy `Telegram` Modal section.
...
8. Copy the `Chat ID` and paste it in Nomploy `Telegram` Modal section.
