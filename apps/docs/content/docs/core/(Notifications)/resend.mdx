...
Resend notifications are a great way to stay up to date with important events in your Nomploy panel. You can choose to receive notifications for specific events or all events.
...
4. Go to Nomploy `Notifications` and select `Resend` as the notification provider.
