...
<Callout type="info">
  Not sure whether you need Swarm nodes, remote servers, or a single Nomploy server? See [Deployment Options](/docs/core/deployment-options) for a comparison.
</Callout>

When you deploy applications in nomploy, all of them run on the same node. If you wish to run an application on a different server, you can use the cluster feature.

The idea of using clusters is to allow each server to host a different application and, using Traefik along with the load balancer, redirect the traffic from the nomploy server to the servers you choose.
...
1. **Vertical Scaling**: This involves adding more resources to the same nomploy server, such as more CPU and RAM.
...
To perform vertical scaling, you need to add more resources to your nomploy server, that is, more CPU and RAM. This is done through your VPS provider.
...
1. nomploy server running (Manager).
2. Have at least one extra server with the same architecture as the nomploy server.
...
You can click the 'Add Node' button, which will display the instructions you need to follow to add your servers as nodes and join them to the nomploy manager node.
...
<Callout type="info">
  **Storage Cleanup Note**: Nomploy does not perform automatic cleanup of storage for workers or other associated nodes that are not the Nomploy server. For automatic cleanup, you can add your node as a remote server and configure cleanups, or create a schedule that performs that cleanup. Additionally, you don't need to perform setup when you only add the node as a remote server. For more information, see the [Remote Servers documentation](/docs/core/remote-servers).
</Callout>
