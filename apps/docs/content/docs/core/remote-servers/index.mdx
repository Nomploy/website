...
<Callout type="info">
  Not sure whether you need remote servers, Swarm nodes, or a single Nomploy server? See [Deployment Options](/docs/core/deployment-options) for a comparison.
</Callout>
...
To use the remote servers feature, you need to have Nomploy UI installed either locally or on a remote server. We recommend using a remote server for better connectivity, security, and isolation, for remote instances we install only a traefik instance.

If you plan to only deploy apps to remote servers and use Nomploy UI for managing deployments, Nomploy will use around 250 MB of RAM and minimal CPU, so a low-resource server should be sufficient.

All the features we have documented previously are supported by Nomploy Remote Servers. The only feature not supported is remote server monitoring, due to performance reasons. However, all functionalities should work the same as when deploying on the same server where Nomploy UI is installed.

## Server Types

Nomploy supports two types of remote servers:
