...
A **Build Server** is a remote server dedicated to building and compiling your applications. Instead of building your application on the deployment server, Nomploy will:
...
1. A Nomploy instance running
2. A remote server for builds (VPS, cloud instance, or dedicated server)
3. SSH access to the build server
4. Docker installed on the build server (or use Nomploy's automatic setup)
...
Navigate to **Dashboard → Remote Servers → Add Server -> Build Type (Build Server)** in your Nomploy dashboard.
...
1. **Build Phase**:
   - Nomploy connects to your build server via SSH
...
2. **Deploy Phase**:
   - Nomploy connects to your deployment server(s)
