...
Nomploy Cloud is the managed version of Nomploy. Instead of installing and maintaining Nomploy on your own server, the **control plane** (UI, database, and management layer) is hosted by us — you just connect your own servers and deploy.

## How it works

With **Self-Hosted** Nomploy, everything runs on a single server: the UI, the database (PostgreSQL), Redis, Traefik, and your applications — all on the same machine. This works well for small setups, but as you scale, your management layer competes for resources with your actual workloads.

**Nomploy Cloud** separates these concerns:

```
┌──────────────────────────────┐       ┌─────────────────────────┐
│       Nomploy Cloud          │       │    Your Server(s)       │
│    (managed by Nomploy)      │       │  (any cloud provider)   │
│                              │       │                         │
│  ┌────────────┐              │       │  ┌───────────────────┐  │
...
Your code and data **never leave your servers**. Nomploy Cloud only manages the orchestration.

## Key benefits

| Benefit | Description |
|---------|-------------|
| **No management overhead** | No need to maintain the Nomploy instance itself — updates, backups, and uptime are handled for you |
...
| **Automatic updates** | Always on the latest version of Nomploy without manual upgrades |
| **Support** | Direct support via email/chat (Startup) or priority SLA (Enterprise) |

## Getting started

1. Register on [Nomploy Cloud](https://app.nomploy.com).
2. Add a server by providing the SSH connection details (IP, port, SSH key).
3. Nomploy Cloud will set up the server automatically (Docker, Traefik, monitoring agent).
...
Nomploy Cloud offers several plans:
...
See the [Pricing](https://nomploy.com/pricing) page for full plan details and feature limits.

## When to use Cloud vs Self-Hosted

| Use Cloud when... | Use Self-Hosted when... |
|-------------------|------------------------|
| You don't want to maintain the Nomploy instance | You want full control over everything |
