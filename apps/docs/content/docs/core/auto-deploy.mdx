---
title: Auto Deploy
description: Automatically deploying your application to Nomploy can be achieved through two primary methods using Webhooks or the Nomploy API. Each method supports various platforms and provides a streamlined deployment process.
---
...
1. **Enable Auto Deploy**: Toggle the 'Auto Deploy' button found in the general tab of your application settings in Nomploy.
...
3. **Configure Your Repository**:
   ...
   - Add the webhook URL provided by Nomploy.
...
- **Branch Matching**: When using Git-based providers (GitHub, GitLab, Gitea, etc.), ensure that the branch configured in Nomploy matches the branch you intend to push to. Misalignment will result in a "Branch Not Match" error.
- **Docker Tags**: For deployments using DockerHub, ensure the tag pushed matches the one specified in Nomploy.
...
5. That's it, now every time you push to your repository, your application will trigger a deployment in nomploy.

The deployment will trigger only if the `Tag` matches the one specified in Nomploy.

## API Method

Deploy your application programmatically using the Nomploy API from anywhere, this is useful when you want to trigger a deployment from a CI/CD pipeline or from a script.

### Steps to Deploy Using API

Steps:

1. **Generate a Token**: Create an API token in your profile settings on Nomploy.
