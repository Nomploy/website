---
title: DNS Providers
description: "Connect Cloudflare or AWS Route53 to create and manage A/CNAME records for your domains without leaving Nomploy."
---
...
DNS Providers let you connect Nomploy to Cloudflare or AWS Route53 so you can browse your zones and create the **A** or **CNAME** record a domain needs, instead of switching to your DNS provider's dashboard by hand.
...
- Records of type **A** and **CNAME** are editable and deletable from Nomploy.
...
- **Content**: labeled **IPv4 Address** for A records or **Target** for CNAME records. For A records, an optional **Fill from server** dropdown auto-fills this with the public IP of the Nomploy panel server or any registered remote server.
- **TTL** (optional): leave empty for the provider's automatic/default TTL.

<Callout type="warn">
  Deleting a record from this screen removes it from the actual DNS provider, not just from Nomploy — the confirmation dialog calls this out explicitly.
</Callout>
