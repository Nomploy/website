---
title: AWS Route53
description: "Connect AWS Route53 to create A/CNAME records for your domains from Nomploy."
---
...
Create an IAM user (or role) with programmatic access and a least-privilege policy for the hosted zones Nomploy should manage:
...
<Callout type="info">
  Route53 is a global service, so there is no region field — Nomploy talks to it directly. There is also no custom endpoint override: this integration only works against real AWS, not LocalStack or other Route53-compatible emulators.
</Callout>
