...
Nomploy offers a UI to connect to any Docker Registry.
...
- **Image Prefix(Optional)**: Useful when using Cluster feature, to tag your images with a prefix eg. `nomploy` will convert to `nomploy/my-app:latest`.
...
Nomploy will default to using its own server.