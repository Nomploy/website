...
3. **Let's Encrypt Certificates**: It's crucial to point the domain to your server's IP **before** adding it in Nomploy. If the domain is added first, the certificate won't be generated, and you may need to recreate the domain or restart Traefik.
...
Then, when creating the domain in Nomploy, specify the service name and port, like this:
...
Nomploy uses two different methods for configuring Traefik routing:
