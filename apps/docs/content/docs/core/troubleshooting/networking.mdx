...
```bash
curl -sSL https://nomploy.com/install.sh | sudo ADVERTISE_ADDR=your-ip sh
```
...
- The Nomploy UI never becomes reachable on port 3000 (Nomploy can't connect to its Postgres service)
...
If you can't switch to a kernel with IPVS support, run Nomploy's services in **DNSRR mode** (DNS round-robin), which bypasses the IPVS-based load balancer entirely.
...
**For an existing installation**, switch the Nomploy services to DNSRR mode:

```bash
docker service update --endpoint-mode dnsrr nomploy-postgres
docker service update --endpoint-mode dnsrr nomploy
```

### Reaching Your Deployed Applications

Applications you deploy through Nomploy are still created as Swarm services in VIP mode. When another container (for example, a `cloudflared` tunnel) needs to reach one of them by name, use the `tasks.<app-name>:<port>` hostname instead of `<app-name>:<port>` — the `tasks.` prefix resolves directly to the container IPs, bypassing the broken VIP load balancer.
...
<Callout type="warning">
  Restarting Docker briefly restarts all running containers, including Nomploy itself, so do this during a low-traffic window. Everything recovers automatically: `nomploy` and `nomploy-postgres` are Swarm services, and `nomploy-traefik` has `--restart always`.
</Callout>
