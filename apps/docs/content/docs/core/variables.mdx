...
description: "Nomploy allows you to create and manage shared and service-level environment variables for your projects and environments."
---
...
Environment variables in Nomploy allow you to:
...
<Callout type="info">
  Nomploy will automatically replace `${{project.DATABASE_URL}}` with the value defined in the project's shared variables.
</Callout>

You can use shared environment variables in all the services available in nomploy.
...
<Callout type="info">
  Nomploy will automatically replace `${{environment.VARIABLE_NAME}}` with the value defined in the environment's variables.
</Callout>
...
<Callout type="info">
  Preview Deployments environments also include a service-level variable called `NOMPLOY_DEPLOY_URL`, which points to the deployment URL of the service.
  It can be used as `${{NOMPLOY_DEPLOY_URL}}` for variables like `APP_URL=https://${{NOMPLOY_DEPLOY_URL}}`.
</Callout>
...
Values are fetched at deploy time and never stored in Nomploy. See [Secrets Providers](/docs/core/secrets-providers) for setup, reference formats and access control.
