---
title: "Schedule Jobs"
description: "Learn how to automate tasks using Nomploy's Schedule Jobs feature"
---


Schedule Jobs in Nomploy allows you to create and manage automated tasks that run on a specified schedule using cron expressions. Each job execution creates a log entry where you can monitor the output and execution status.

## Job Types

Nomploy supports four types of scheduled jobs:
...
4. **Nomploy Server Jobs**: Execute tasks at the container level within the Nomploy container. These jobs can interact with commands inside the Nomploy container (e.g., `docker ps`, `docker image prune`), but they are not executed directly on the host system

## Container-based Jobs (Application and Compose)

For application and compose jobs, you can run single commands that will be executed inside the target container. Nomploy internally uses Docker exec to run these commands:
...
## Server-based Jobs (Server and Nomploy Server)
...
Nomploy Server Jobs are executed at the container level within the Nomploy container. This means:

- Commands run inside the Nomploy container environment
...
- Jobs do not execute directly on the host system, but within the containerized Nomploy environment
...
This command will run inside the Nomploy container and can interact with Docker to clean up images.
...
You can create scripts to backup databases that aren't natively supported by Nomploy. Here's an example structure for a custom backup script:
