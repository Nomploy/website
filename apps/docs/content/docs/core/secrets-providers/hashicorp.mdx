...
description: "Use HashiCorp Vault or OpenBao KV v2 secrets in your Nomploy environment variables."
---

import { Callout } from "fumadocs-ui/components/callout";

Nomploy talks to the KV version 2 secrets engine. The same provider works with **HashiCorp Vault** and **[OpenBao](https://openbao.org)** (the open-source fork) — their APIs are identical.
...
- **Vault URL**: the address of your Vault/OpenBao server, e.g. `https://vault.example.com:8200`. It must be reachable from your Nomploy server.
...
<Callout type="info">
  Create a token from a policy that only grants read/list on the paths Nomploy needs, for example:
