...
description: "Use Azure Key Vault secrets in your Nomploy environment variables."
---

import { Callout } from "fumadocs-ui/components/callout";

Nomploy authenticates with an **App Registration** (service principal) using the OAuth2 client credentials flow.

## Create an App Registration

1. In the Azure Portal, go to **App registrations → New registration**, name it (e.g. `nomploy`) and register it with the defaults — no redirect URI needed.
...
- **RBAC permission model** (recommended): go to **Access control (IAM) → Add role assignment**, pick the **Key Vault Secrets User** role (read-only) and assign it to the `nomploy` app.
