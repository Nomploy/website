...
Secrets Providers let you connect Nomploy to an external secret manager and reference its secrets from any environment variable editor:
...
Secrets are fetched from the provider **at deploy time** and injected into the prepared environment. The values are never stored in the Nomploy database — your vault remains the single source of truth, and what members see in the env editor is the reference, not the value.
...
- **Values never touch the Nomploy database or the UI.** Members with access to a service see `${{vault.prod-vault.DB_URL}}` in the editor, not the value.
