---
title: Doppler
description: "Use Doppler secrets in your Nomploy environment variables."
---
...
<Callout type="info">
  A provider points at a single Doppler config. For several configs (e.g. `dev` and `prd`), create one provider per config and assign each one to the matching Nomploy environments.
</Callout>
