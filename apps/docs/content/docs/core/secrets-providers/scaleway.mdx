...
description: "Use Scaleway Secret Manager secrets in your Nomploy environment variables."
---
...
Nomploy authenticates with a Scaleway **API key** scoped to the project your secrets live in.
...
<Callout type="warn">
  The Secret Key is the credential, there is no separate access key ID — Nomploy authenticates with it alone via the `X-Auth-Token` header.
</Callout>
