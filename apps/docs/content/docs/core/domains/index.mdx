---
title: Domains
description: This section outlines how to configure domains for your applications in Nomploy, ensuring that your applications are accessible via custom URLs.
---

import { Callout } from 'fumadocs-ui/components/callout';
import { Link } from 'next/link';

Nomploy Provide 2 ways to add a domain to your service:
...
If you don't have a domain, you can use our integrated free domains from Traefik.me in the Nomploy panel, without doing any configuration however is only for HTTP, if you want to have a HTTPS domain you need to buy a domain from one of the providers above.
...
Nomploy handles domains differently depending on whether you're using Applications or Docker Compose:

### Applications

For **Applications**, Nomploy creates a unique configuration file for each domain you create or update. These files are managed automatically and can be viewed in:

- **Traefik File System**: Accessible through the Nomploy UI
...
http:
  routers:
    nomploy-app-name-router-1:
      rule: Host(`your-domain.com`)
      service: nomploy-app-name-service-1
      middlewares:
        - redirect-to-https
      entryPoints:
        - web
  ...
  services:
    nomploy-app-name-service-1:
      loadBalancer:
        servers:
          - url: http://nomploy-app-name:3000
        passHostHeader: true
```
...
- **Hot Reload**: Nomploy uses Traefik's [File Provider](https://doc.traefik.io/traefik/reference/install-configuration/providers/others/file/) which supports hot reloading, so configuration changes are applied automatically
...
- **Host**: The domain name that you want to link to your application (e.g., `api.nomploy.com`).
...
Nomploy uses Traefik middlewares to modify request paths before they reach your application. These powerful tools allow you to create flexible routing configurations that match your application's expected URL structure.
...
**Example:**
- Domain: `api.nomploy.com`
- Path: `/v1`
- Internal Path: `/backend/api`
- Request: `api.nomploy.com/v1/users`
...
**Example:**
- Domain: `app.nomploy.com`
- Path: `/dashboard`
- Strip Path: Enabled
- Request: `app.nomploy.com/dashboard/settings`
...
**Example:**
- Domain: `service.nomploy.com`
- Path: `/public`
- Strip Path: Enabled
- Internal Path: `/app/v2`
- Request: `service.nomploy.com/public/api/users`
- Processing:
...
Create the domain in nomploy, using the host `www.example.com`

Now in nomploy, you can go to application -> advanced -> redirects (Select the preset www to non-www) and click on save.
