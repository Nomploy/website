...
description: Learn how to use Cloudflare Tunnels to securely expose your Nomploy applications without opening ports on your server.
---
...
Before setting up Cloudflare Tunnels with Nomploy, ensure you have:
...
<Callout type="warn">
  When using Cloudflare Tunnels, you should **disable Let's Encrypt** in Nomploy and use HTTP instead of HTTPS for internal connections. Cloudflare handles SSL/TLS termination at their edge.
</Callout>
...
- Give your tunnel a name (e.g., `nomploy-tunnel`)
...
For Cloudflare Tunnels to work properly with Nomploy:
...
### Step 3: Create Cloudflare Service in Nomploy
...
Nomploy uses **Traefik** as its reverse proxy to route traffic to your applications. When configuring Cloudflare Tunnels, you have two options:
...
- **Benefits**:
  ...
  - Leverage all Nomploy domain configurations (redirects, path rewrites, etc.)
  - Traefik automatically routes based on the domain you configured in Nomploy
...
- Configure:
  ...
  - **Service**:
    - **Type**: HTTP
    - **URL**: `nomploy-traefik:80`
- Click **Save**

<Callout type="info">
  With this setup, Traefik will route the request to the correct application based on the domain you configured in Nomploy's domain settings.
</Callout>
...
- Configure the public hostname with:
  - **Service**:
    - **Type**: HTTP
    - **URL**: `appName:port` (e.g., `nomploy:3000`, `my-app:8080`)

Note: The app name in Nomploy is shown under the service name, usually formatted as `project-serviceName-hash`

<Callout type="warn">
  When using direct access, you bypass Traefik completely. Domain configurations in Nomploy won't apply, and you'll need to configure each container separately in Cloudflare.
</Callout>
...
This allows all subdomains (`app1.example.com`, `app2.example.com`, etc.) to route through Traefik, which then directs traffic to the appropriate container based on your Nomploy domain configurations.

<Callout type="info">
  With wildcard routing, you only need ONE public hostname in Cloudflare Tunnel. Traefik handles routing to different apps based on the domain configured in Nomploy.
</Callout>

## Using Cloudflare Access (Zero Trust) with Nomploy

If you protect your Nomploy dashboard with **Cloudflare Access** (Zero Trust), there are a few extra steps required to make login work correctly.

### Why login hangs behind Cloudflare Access

When Cloudflare Access sits in front of Nomploy, the login request (`POST /api/auth/sign-in/email`) may appear to hang indefinitely even with valid credentials. Invalid credentials still return a `401` immediately, which makes it easy to miss this issue.

The root cause is that Better Auth (Nomploy's auth library) validates the `Origin` header of every request. When the request comes through Cloudflare Access, the origin may not match any of the trusted origins Nomploy knows about, so the request is silently dropped.
...
Set the `BETTER_AUTH_TRUSTED_ORIGINS` environment variable to include all origins that can reach Nomploy:

```bash
BETTER_AUTH_TRUSTED_ORIGINS=http://localhost:3000,http://<internal-ip>:3000,https://nomploy.example.com
```

Replace `nomploy.example.com` with your actual public domain.

**3. Configure Web Server settings in the Nomploy UI**

Go to **Settings → Server** and set:
- **Host**: `nomploy.example.com` (your public domain)
- **HTTPS**: enabled

This ensures Nomploy includes your domain as a trusted origin automatically.

<Callout type="info">
  After applying these changes, redeploy the Nomploy service so the new environment variables take effect.
</Callout>

<Callout type="warn">
  If you access Nomploy through multiple origins (public domain, internal IP, Tailscale), make sure all of them are listed in `BETTER_AUTH_TRUSTED_ORIGINS`.
</Callout>