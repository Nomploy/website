...
description: "Nomploy Server vs Remote Servers vs Swarm Nodes: choose where your applications run."
---
...
3. **Swarm Nodes (Cluster)**: servers joined to the Nomploy server's Docker Swarm to replicate applications across machines.
...
The [Cluster](/docs/core/cluster) feature joins additional servers as worker or manager nodes of the Nomploy server's Docker Swarm. Traefik on the manager load-balances traffic to the nodes where your application replicas run.

- **Replication**: run multiple replicas of the same application across machines, with load balancing handled by Traefik.
- **Requirements**: a Docker registry (images must be pushed so other nodes can pull them) and servers with the same architecture as the Nomploy server.
...
|                                  | Nomploy Server | Remote Servers            | Swarm Nodes (Cluster)   |
| -------------------------------- | -------------- | ------------------------- | ----------------------- |
...
<Callout type="info">
  These options are not mutually exclusive. A common setup is a small server running only the Nomploy UI, with applications deployed to remote servers. You can also add a Swarm node as a remote server to configure storage cleanups on it.
</Callout>
