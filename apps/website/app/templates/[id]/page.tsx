...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
	...
	const title = `Deploy ${template.name} on Nomploy`;
	const description = `${template.description} Deploy ${template.name} on your own server with one click using Nomploy's open source template.`;
	const url = `https://nomploy.com/templates/${template.id}`;
...
export default async function TemplatePage({ params }: Props) {
	...
	const jsonLd = {
		...
		url: `https://nomploy.com/templates/${template.id}`,
		image: logoUrl,
		...
		<div className="container mx-auto max-w-5xl px-4 py-12 mt-14">
			...
			<section id="deploy" className="mt-12 scroll-mt-24">
				<h2 className="text-2xl font-semibold">
					Deploy {template.name} on Nomploy
				</h2>
				<ol className="mt-4 list-decimal space-y-2 pl-6 text-muted-foreground">
					...
					<li>
						In your Nomploy panel, open a project and click{" "}
						<strong>Create Service → Template</strong>.
					</li>
					<li>
						Search for <strong>{template.name}</strong> and click{" "}
						<strong>Create</strong> — Nomploy configures domains, environment
			...
			<section className="mt-12 flex flex-col items-start gap-3 rounded-lg border border-border bg-muted/30 p-6">
				...
				<p className="text-muted-foreground">
					Nomploy is a free, open source deployment platform. Deploy{" "}
