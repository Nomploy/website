...
# Usage with curl (export first): export NOMPLOY_VERSION=canary && curl -sSL https://nomploy.com/install.sh | sh
# Usage with curl (export first): export NOMPLOY_VERSION=latest && curl -sSL https://nomploy.com/install.sh | sh
# Usage with curl (bash -s): NOMPLOY_VERSION=canary bash -s < <(curl -sSL https://nomploy.com/install.sh)
# Usage with curl (default): curl -sSL https://nomploy.com/install.sh | sh (detects latest stable version)
# Usage with bash: NOMPLOY_VERSION=canary bash install.sh
# Usage with bash: NOMPLOY_VERSION=latest bash install.sh
# Usage with bash: bash install.sh (detects latest stable version)
detect_version() {
    local version="${NOMPLOY_VERSION}"
    ...
    if [ -z "$version" ]; then
        ...
        version=$(curl -fsSL --connect-timeout 10 -o /dev/null -w '%{url_effective}\n' \
            https://github.com/nomploy/nomploy/releases/latest 2>/dev/null | \
            sed 's#.*/tag/##')
        ...
        # like nomploy/nomploy:https://... Accept only values that look like a
...
install_nomploy() {
    ...
    DOCKER_IMAGE="nomploy/nomploy:${VERSION_TAG}"
    
    echo "Installing Nomploy version: ${VERSION_TAG}"
    if [ "$(id -u)" != "0" ]; then
    ...
    if ss -tulnp | grep ':3000 ' >/dev/null; then
        ...
        echo "Nomploy requires port 3000 to be available. Please stop any service using this port." >&2
        exit 1
    fi
    ...
    if [ -z "$advertise_addr" ]; then
        ...
        echo "Example: curl -sSL https://nomploy.com/install.sh | sudo ADVERTISE_ADDR=192.168.1.100 sh"
        exit 1
    fi
    ...
    docker network rm -f nomploy-network 2>/dev/null
    docker network create --driver overlay --attachable nomploy-network

    echo "Network created"

    mkdir -p /etc/nomploy

    chmod 777 /etc/nomploy

    # Generate secure random password for Postgres
    POSTGRES_PASSWORD=$(generate_random_password)

    # Store password as Docker Secret (encrypted and secure)
    echo "$POSTGRES_PASSWORD" | docker secret create nomploy_postgres_password - 2>/dev/null || true

    # Generate secure auth secret for Better Auth
    AUTH_SECRET=$(openssl rand -hex 32)

    # Store auth secret as Docker Secret (encrypted and secure)
    echo "$AUTH_SECRET" | docker secret create nomploy_auth_secret - 2>/dev/null || true
    ...
    echo ""
    printf "${GREEN}Congratulations, Nomploy is installed!${NC}\n"
...
update_nomploy() {
    ...
    echo "Updating Nomploy to version: ${VERSION_TAG}"

    # Pull the image
    docker pull $DOCKER_IMAGE

    # Update the service
    docker service update --image $DOCKER_IMAGE nomploy

    echo "Nomploy has been updated to version: ${VERSION_TAG}"
}
